    ...
    <div class="body-hoja">
        ...
        <div class="informacion-1">
            <div class="info">
                <form method="POST">
                    @csrf
                    <input type="date" name="texto">
                    <input type="hidden" name="opcion" value="buscar_fecha_h">
                    <input type="submit" value="🔎">
                </form>
            </div>
            <div class="info">
                <form method="POST" action="{{ route('pedido_entrega') }}">
                    @csrf
                    <select name="opcion">
                        <option value="buscar_orden_h">ID Orden</option>
                        <option value="buscar_cliente_h">ID Cliente</option>
                    </select>
                    <input type="number" name="texto" placeholder="¿Qué desea buscar?" required min="1">
                ...
                <tbody>
                    @foreach($result as $row)
                        <tr>
                            ...
                            <td>{{ $row->estado_hoja }}</td>
                            <td>{{ $row->fecha_pedido }}</td>
                            <td>{{ $row->fecha_entrega }}</td>
                            <td><a href="{{ route('ver_mas', $row->orden_id) }}" class="hoja-tabla-ver">Ver Detalles</a></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
